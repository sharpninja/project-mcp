...
**Hierarchy:** Enterprise (top-level ownership) → Project → work items (and tasks, same entity), milestones, releases. See [00 — Definitions](00-definitions.html). The project methodology does not include sprints.
...
- **Tech stack:** key-value mapping (e.g. `languages: ["TypeScript"]`, `frameworks: ["Node"]`). Values are string arrays. Projects may have **standards** assigned to them (project-level standards); see Standards below.
...
- **Asset type taxonomy (starter):** `document`, `diagram`, `image`, `video`, `audio`, `code`, `dataset`, `other`. **URN scheme:** `urn:asset:{type}:{guid}` (e.g. `urn:asset:diagram:3f1c...`) or a URL; default_icon_urn follows the same `urn:asset-icon:{type}` pattern.
...
- **Team semantics:** Team membership is **direct only** (no nested teams). If an item is assigned both to a team and an individual, the **individual assignment takes precedence**; queues and “assigned to me” views include the union but de-duplicate items.
...
- **Completion roll-up:** When **all child items** under a work item are complete (child work items have **state = Complete** and child tasks have **status = done**), the parent work item is updated to **state = Complete**; this check applies **recursively** to ancestors.
- **Approval:** When a work item or milestone enters **Approval**, record an **approval_events** entry (entity_type, entity_id, approved_by resource_id, approved_at). The item transitions to **Complete** only after an approval event is recorded.
...
- **Scope:** Applies to all entity tables (enterprise, project, milestone, release, work_item, requirement, resource, domain, system, asset, issue, standards, keywords, and junction tables such as project_dependencies, item_dependencies, work_item_resource_effort, milestone_scope_milestones, entity_keywords, etc.). **All columns** of these tables are in scope; no field may be excluded from change tracking.
...
- **Retention and access:** **Audit logging is in scope for V1:** the application must support both (1) **recording** every change with session_id, resource_id, and correlation_id, and (2) a **read-only audit/history API** for SUDO/admin users to query by entity, date range, session, or resource (see [06 — Tech Requirements](06-tech-requirements.html)). Retention policy (e.g. how long to keep audit rows) is an operational concern.
...
- **Tables** (or equivalent schema). Change tracking (see above) must be enabled for all entity tables and all fields. Each entity has `id` (GUID, PK) and `display_id` (slug, unique); see [08 — Identifiers](08-identifiers.html). For entities with polymorphic parents, the child’s slug is derived from the **parent’s slug**; slug changes and all descendant slug updates must run in a **single transaction** (roll back entire change on failure).
  ...
  - **project** — id (GUID), display_id (slug, e.g. E1-P001), enterprise_id (FK), metadata (name, description, status, createdAt, updatedAt), tech stack (JSONB or related table).
