...
2. **Data layer** — Model types for Project, **WorkItem (level = Work | Task)**, Milestone, Release; PostgreSQL schema (tables/migrations); data access (e.g. EF Core, Dapper, or ADO); resolve project root from env or current directory for **doc_read**.
